...
//	#else
	PCICR &= ~(1 << PCIE1); // disable pcint1 interrupt  //bitClear(EIMSK, INT0);
...
//	bitSet(EIMSK, PCIE0);
//	#else
	 PCICR |= (1 << PCIE1); // disable pcint1 interrupt //bitSet(EIMSK, INT0);
//	bitSet(EIMSK, PCIE1);
//	#endif
//	#else
//	bitSet(EIMSK, INT0);
...
//	bitSet(SS_DDR, cs_pin); in setup loop!
		...
		DDR_SPI |= (1 << BIT_MOSI) | (1 << BIT_SCK);  // SDI, SCK,  CS output in setup loop!
...
void rf12_sendStart (uint8_t hdr, const void* ptr, uint8_t len) {
	LED_868SEND_PORT |= (1 << LED_868SEND_BIT); // set led ON
	rf12_len = len;
...
void rf12_interruptcontrol () {
	//#if PINCHG_IRQ
	   	...
	   	if ((nodeid & NODE_ID) != 0) {
		    DDR_RFM_IRQ &= ~(1 << BIT_RFM_IRQ);  // input
		    PORT_RFM_IRQ |= (1 << BIT_RFM_IRQ);  // pull-up //  bitSet(PORTB, 3);
		    PCMSK1 |= (1 << BIT_RFM_IRQ);  // pin-change //  bitSet(PCMSK0, 3);
		    PCICR |= (1 << PCIE1);  // enable //   bitSet(PCICR, PCIE0);
		    } else {
		    PCMSK1 &= ~( 1 << BIT_RFM_IRQ);  // pin-change //   bitClear(PCMSK0, RFM_IRQ - 8);
	    }
