...
void RFM12B::ReceiveStart() {
	...
	//blockInterrupts();
	SetLowDutyOnOff(1);
	XFERSlow(RF_RECEIVER_LOWDUTY_ON);
...
if(value){ // set low duty on in lowduty register 15
	lowdutyregister |= (1 << 0);
	Control(lowdutyregister); // set bit 0 (enable) to 1
		
}else{ // set low duty off in lowduty register 15
	lowdutyregister &= ~(1<<0);  
	Control(lowdutyregister); // set bit 0 (enable) to 0
}
